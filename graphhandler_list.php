...
/**
 *   	\file       graphhandler_list.php
 *		\ingroup    graphhandler
 *		\brief      List page for graphhandler
 */
...
require_once __DIR__.'/class/graphhandler.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('graphhandler', 'graphhandler', 'write');
	$permissiontodelete = $user->hasRight('graphhandler', 'graphhandler', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("graphhandler")) {
	accessforbidden('Module graphhandler not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'GraphHandler';
	$objectlabel = 'GraphHandler';
	$uploaddir = $conf->graphhandler->dir_output;
...
$title = $langs->trans("GraphHandlers");
//$help_url = "EN:Module_GraphHandler|FR:Module_GraphHandler_FR|ES:Módulo_GraphHandler";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/graphhandler/graphhandler_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/graphhandler/graphhandler_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendGraphHandlerRef";
$modelmail = "graphhandler";
$objecttmp = new GraphHandler($db);
